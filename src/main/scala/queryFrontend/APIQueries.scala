...
object APIQueries {
  ...
  def getProgramIdsBySiliconData(
    success: Boolean = true,
    didTimeout: Boolean = false,
    minRuntime: Long = 0,
    maxRuntime: Long = Long.MaxValue
  ): Seq[Long] = {
    val ids =
      try {
        val queryObj = Obj(
        ...
        val json = getResponseObj(s"$API_HOST/program-ids-by-silicon-data", queryObj)
        read[Seq[Long]](json("programIds").arr)
      } catch {
        case e: Exception => e.printStackTrace(); Seq()
      }
  ...
  def getProgramIdsByCarbonData(
    ...
    val ids =
      try {
        ...
        val json = getResponseObj(s"$API_HOST/program-ids-by-carbon-data", queryObj)
        read[Seq[Long]](json("programIds").arr)
      } catch {
        case e: Exception => e.printStackTrace(); Seq()
      }
